if [ ! -f $HOME/.ista_timehelper.txt  ]; then
    ...
    echo $PS1 > .ista_timehelper.txt # Old PS1 value
    echo "$(date +%s)" >> .ista_timehelper.txt # Starting time
    PS1="[ON THE CLOCK]$ "
else
    echo "Hidden file ista_timehelper.txt found!"
    echo "Deleting file, reseting PS1 shell variable, and calculating time now..."
    oldPS1="$(head -1 .ista_timehelper.txt)"
    startTime="$(tail -1 .ista_timehelper.txt)"
    endTime="$(date +%s)"
    echo "oldPS1: $oldPS1"
